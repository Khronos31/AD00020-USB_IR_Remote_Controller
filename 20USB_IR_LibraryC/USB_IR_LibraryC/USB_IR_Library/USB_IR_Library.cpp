...
int lua_sendUSBIR(lua_State* L)
{
	auto hHandle = GetModuleHandle(0);
	auto usbHandle = openUSBIR(hHandle);
	auto format_type = luaL_checkinteger(L, 1);
	auto data = luaL_checkstring(L, 2);
	auto code = reinterpret_cast<unsigned char*>(const_cast<char*>(data));
	auto code_len1 = luaL_checkinteger(L, 3);
	auto code_len2 = luaL_checkinteger(L, 4);

	auto send_result = writeUSBIRex(usbHandle, format_type, code, code_len1, code_len2);;
	auto result = closeUSBIR(usbHandle);
