// USB IR REMOCON KIT
//	ver 2.1.2	2018．09．12 一部PCで接続するとフリーズする対策 キーボードデータの受信処理追加
//	ver 2.1.1	2017．04．07 バグFix 赤外線送信されない場合がある不具合修正
    ...
    if(!HIDRxHandleBusy(USBOutHandle))				//Check if data was received from the host.
    {
        ...
        switch(ReceivedDataBuffer[0])
        {
            ...
            case 0x60:	/* 赤外線出力データを受信する */
				...
				// 送信赤外線データの有無を先頭１バイトで判断しているため、後ろからコピーする 2017.04.07
				for(fi = (OUTBUFFER_SIZE - 1); fi >= 0; fi--){
            ...
            case 0x61:	/* 赤外線出力データを受信する 拡張版 */
				...
				// 送信赤外線データの有無を先頭１バイトで判断しているため、後ろからコピーする 2017.04.07
				for(fi = (OUTBUFFER_SIZE_EX - 1); fi >= 0; fi--){
					uc_out_buff[fi] = ReceivedDataBuffer[fi+1];
				}
                break;
        }
